...
$position = array("-- selectionner --","Animatrice","Résident");

$deparment = array("--selectionner --","Accountancy","Marketting","Purchasing","Computing");
  ...
  if(isset($_POST["submit"])){
    ...
    if($first_name!="" && $last_name!="" && $user_password!="" && $user_position!="" && $user_department!="" && $security_key!=""){
      ...
      if($check_validity_status["status"] == "not_used"){
        ...
        "<script>alert('Désolé, vous ne pouvez pas être enregistré, contactez l'administrateur')</script>";
      }
      }else{
        echo "<script>alert('Vous êtes déjà inscrit')</script>";
      }
    }else{
        echo "<script>alert('Veuillez contacter l'administrateur pour obtenir une clé de sécurité valide')</script>";
    }
  }else{
    echo "<script>alert('Veuillez remplir tous les espaces fournis')</script>";
  }
}
 ?>
<!--<!DOCTYPE html>-->
...
<body>
  ...
  <div class="flex-row">
    <input id="fname" class='lf--input' placeholder='Entrer votre Prénom' type='text' name="txtfname">
  </div>
  <div class="flex-row">
    <input id="lname" class='lf--input' placeholder='Entrer votre Nom' type='text' name="txtlname">
  </div>
  ...
  <div class="flex-row">
    <input id="password" class='lf--input' placeholder='Mot de passe' type='password' name="txtpassword">
  </div>

  <div class="flex-row">
    <input id="password" class='lf--input' placeholder='Entrer la clé de sécurité' type='password' name="txtsecurity_key">
  </div>
  <input class='lf--submit lf--submit-adjusted' type='submit' value='Inscription' name="submit" id="btnSubmit">

  <p style="font-size:.6em;color:#2dc6bf;text-align:center;cursor:hand"><u><a href="index.php">Se connecter</a></u></p>
